...
Tools are defined using the `AgentTool` interface from `@sweny/providers/agent-tool`:

```typescript
import type { AgentTool, ToolResult } from "@sweny/providers/agent-tool";
```
...
```typescript
import type { ToolPlugin, PluginContext } from "@sweny/agent";
```
