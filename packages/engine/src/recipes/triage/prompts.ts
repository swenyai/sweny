...
export function issueLink(name: string | undefined, identifier: string, url: string): string {
  if (name === "github-issues") return `Closes ${identifier}`;
...
function issueTrackerApiInstructions(name?: string): string {
  switch (name) {
    case "linear":
      return `### Linear API
