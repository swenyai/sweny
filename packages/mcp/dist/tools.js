...
export function registerTools(server) {
    server.tool("sweny_list_workflows", "List available SWEny workflows in the current project. Returns built-in workflows (triage, implement, seed-content) and any custom workflows from .sweny/workflows/. Note: only triage and implement are runnable via sweny_run_workflow.", {
    ...
    server.tool("sweny_run_workflow", "Execute a SWEny workflow. Triage discovers and investigates alerts, then creates issues/PRs. Implement takes an issue ID and writes a fix. Requires .sweny.yml config and credentials in env. Can take several minutes.", {
        workflow: z.enum(["triage", "implement"]).describe("Which workflow to run"),
        input: z
            .string()
            .optional()
            .describe("For implement: issue ID or URL (required). For triage: not needed (discovers alerts automatically)."),
