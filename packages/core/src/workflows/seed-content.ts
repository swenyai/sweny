 ...
 *
 * A DAG that audits a Supabase-backed educational platform for content gaps,
 * generates high-quality content using Claude, validates it, and publishes it.
 *
 * Designed for solo operators bootstrapping educational SaaS platforms.
 * Uses Claude (via your existing subscription) for generation instead of
 * burning API credits on external AI gateways.
 *
 * @example
 * ```ts
 * import { execute, ClaudeClient, createSkillMap, supabase } from '@sweny-ai/core'
 * import { seedContentWorkflow } from '@sweny-ai/core/workflows'
 *
 * const skills = createSkillMap([supabase])
 * const claude = new ClaudeClient()
 *
 * const results = await execute(seedContentWorkflow, {
 *   contentType: 'worksheet',           // 'worksheet' | 'game' | 'lesson'
 *   subject: 'math',
 *   topic: 'fractions',
 *   difficulty: 'easy',
...
export const seedContentWorkflow: Workflow = {
  id: "seed-content",
  ...
  nodes: {
    ...
    audit: {
      ...
      instruction: `You are auditing an educational platform's content library.
...
Prioritize gaps where a subject has ZERO content of a given type.`,
        ...
        properties: {
          plan: {
            ...
            items: {
              ...
              required: ["contentType", "subject", "topic", "difficulty", "count"],
            },
          },
          summary: { type: "string" },
        },
    ...
    generate: {
      ...
      instruction: `You are an expert educator creating content for children ages 5-14.
...
**For worksheets:**
Generate a JSON array of problems. Each problem needs: "problem", "answer", "format" (answer-line | multiple-choice | write-on).
Requirements:
...
Table schemas:
- worksheets: id (auto), title, subject, topic, difficulty, grade_level, num_questions, problems (jsonb), status='published', source='ai-seed'
- lessons: id (auto), title, subject, difficulty, grade_level, lesson_type, html_code, css_code, js_code, status='published', source='ai-seed'
- games: id (auto), title, subject, difficulty, grade_level, game_type, html_code, css_code, js_code, status='published', source='ai-seed'
...
Return a summary of what was generated and inserted.`,
      ...
      output: {
        ...
        required: ["generated", "totalGenerated"],
      },
    },
    ...
    validate: {
      ...
      instruction: `Review the content that was just generated and inserted.
...
Return a quality report.`,
      ...
      output: {
        ...
        required: ["totalReviewed", "passed", "flagged", "summary"],
      },
    },
    ...
    report: {
      ...
      instruction: `Compile a final report of the content generation run.
