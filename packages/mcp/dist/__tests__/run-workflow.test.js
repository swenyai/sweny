...
describe("runWorkflow", () => {
    it("builds correct CLI args for triage", async () => {
        ...
        expect(mockSpawn).toHaveBeenCalledWith(bin, ["triage", "--json"], expect.objectContaining({ cwd: expect.any(String), stdio: ["ignore", "pipe", "pipe"] }));
    ...
    it("builds correct CLI args for implement with issue ID", async () => {
        ...
        expect(mockSpawn).toHaveBeenCalledWith(bin, ["implement", "ABC-123", "--json"], expect.objectContaining({ stdio: ["ignore", "pipe", "pipe"] }));
        proc.stdout.emit("data", Buffer.from('{"ok": true}'));
        proc._emit("close", 0);
        const result = await promise;
        expect(result.success).toBe(true);
