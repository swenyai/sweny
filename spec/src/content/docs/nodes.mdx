...
The optional `model` field selects which AI model executes a node. It is the execution-time counterpart to the judge `model` / `judge_model` fields, which select the model for [evaluation](#judge-mechanics).
...
- **MUST** forward the resolved model to the AI invocation (step 6 of the [Node Execution Sequence](/execution)) when any layer specifies one.
