# Create Channel Interface and Types

## Context
The agent package at `packages/agent` is being refactored from Slack-only to channel-agnostic. This task creates the core Channel interface that all adapters (Slack, CLI, Discord, etc.) will implement.

## Files to Create

### `packages/agent/src/channel/types.ts`
...
### `packages/agent/src/channel/index.ts`
Barrel export all types from types.ts.

## Verification
- `npm run typecheck --workspace=packages/agent` passes
- No existing code is broken (purely additive)
