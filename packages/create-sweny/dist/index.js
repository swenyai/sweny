...
function collectCreds(providers) {
    const seen = new Map();
...
function buildSwenyYml(sc, obs, it) {
    const lines = [
    ...
    if (obs)
        lines.push(`observability-provider: ${obs}`);
...
async function main() {
    ...
    if (dirArg && dirArg !== ".") {
        ...
        if (p.isCancel(dir))
            cancel();
        targetDir = path.resolve(dir.trim() || ".");
    }
    ...
    if (p.isCancel(obsRaw))
        cancel();
    const observability = obsRaw === "__none" ? null : obsRaw;
    ...
    fs.writeFileSync(configPath, buildSwenyYml(sourceControl, observability, issueTracker));
