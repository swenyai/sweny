...
- **`rules`** — A [Source](/sources) containing directives that MUST be prepended to every node's instruction. Use this for organization-wide policies or run-specific constraints.
- **`context`** — A [Source](/sources) containing background information prepended to every node's instruction. Use this for situation-specific details.

Both `rules` and `context` can be inline text, file paths, or URLs. They resolve eagerly before any node executes. See [Nodes — Input Augmentation](/nodes#input-augmentation) for how they integrate into node instructions.
