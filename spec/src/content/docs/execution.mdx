...
6. **Invoke the AI model** with: instruction, context, tools (from step 2 + MCP tools from step 3), and output schema (if present). The model is resolved per [Model Selection](/nodes#model-selection) (`node.model` → `workflow.model` → executor default); when none is specified the executor uses its implementation-defined default.
