...
| `model`         | string                                                           | OPTIONAL | impl&#8209;defined | Default execution model for every node. Overridable per-node. Free-text passthrough. See [Nodes — Model Selection](/nodes#model-selection). |
