...
| L5 | Root `docs/` carries stale terminology | ✅ Resolved | Banner-archived/-deprecated files removed in #174 (architecture.md) and the `docs/sweep-stale` PR (provider-authoring, recipe-authoring, self-hosted-worker, studio, mcp-servers, recipes/). Canonical reference docs live at docs.sweny.ai. Only `brand-guide.md` and `hardening/` remain in `docs/`. |
