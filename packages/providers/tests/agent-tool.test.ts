...
describe("agentTool factory", () => {
  it("creates an AgentTool with name, description, schema, and execute", () => {
    const tool = agentTool("test_tool", "A test tool", { input: z.string() }, async () => ({
  ...
  it("executes the handler and returns ToolResult", async () => {
    const tool = agentTool("echo", "Echoes input", { text: z.string() }, async (args) => ({
  ...
  it("supports isError in ToolResult", async () => {
    const tool = agentTool("fail", "Always fails", {}, async () => ({
  ...
  it("works with empty schema", () => {
    const tool = agentTool("no_args", "Takes no arguments", {}, async () => ({
  ...
  it("satisfies the AgentTool interface", () => {
    const tool: AgentTool = agentTool("typed", "Typed tool", { value: z.string() }, async (args) => ({
  ...
  it("handler receives Record<string, unknown> args", async () => {
    ...
    const tool = agentTool("capture", "Captures args", { a: z.string(), b: z.number() }, async (args) => {
      receivedArgs = args;
